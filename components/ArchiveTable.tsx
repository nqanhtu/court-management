...
export default function ArchiveTable({ files, onEdit, onDelete }: ArchiveTableProps) {
    ...
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="bg-white p-4 border-b border-slate-200 flex flex-wrap items-center gap-3 shrink-0">
        ...
        <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
          <span className="text-xs font-medium text-slate-500 uppercase tracking-wide">Lọc:</span>
        </div>
            ...
            {paginatedFiles.length === 0 ? (
              <tr>
                <td colSpan={12} className="px-6 py-8 text-center text-slate-500">
                  {filteredFiles.length === 0 && files.length > 0 ? "Không tìm thấy kết quả phù hợp." : "Chưa có hồ sơ nào."}
                </td>
              </tr>
            ) : (
              paginatedFiles.map((file) => (
                <tr key={file.id} className="hover:bg-indigo-50/50 transition-colors group">
                  <td className="px-4 py-2.5 pl-6 font-medium text-slate-900 group-hover:text-indigo-700">{file.code}</td>
                  <td className="px-4 py-2.5 text-slate-600">{file.pageCount || "-"}</td>
                  <td className="px-4 py-2.5 text-slate-600">
                    {file.startDate ? format(new Date(file.startDate), "dd/MM/yyyy") : "-"}
                  </td>
                  <td className="px-4 py-2.5 text-slate-600">{file.type}</td>
                  <td className="px-4 py-2.5 text-slate-600 max-w-xs truncate" title={file.title}>{file.title}</td>
                  <td className="px-4 py-2.5 text-slate-600">{file.box || "-"}</td>
                  <td className="px-4 py-2.5 text-slate-600">{file.mlhs || "-"}</td>
                  <td className="px-4 py-2.5 text-slate-600">{file.mlvb || "-"}</td>
                  <td className="px-4 py-2.5 text-slate-600">{file.retention || "-"}</td>
                  <td className="px-4 py-2.5 text-slate-400 italic truncate max-w-[100px]">{file.note || "-"}</td>
                  <td className="px-4 py-2.5">
                    ...
                    {!["IN_STOCK", "BORROWED", "LOST"].includes(file.status) && (
       ...
       {/* Pagination */}
        <div className="p-3 border-t border-slate-100 bg-white flex items-center justify-between shrink-0">
           <span className="text-xs text-slate-500">
             Hiển thị {(currentPage - 1) * ITEMS_PER_PAGE + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, filteredFiles.length)} trên {filteredFiles.length} bản ghi
           </span>
