...
async function upsertFile(data: {
  ...
  await prisma.fileIndex.upsert({
    ...
    update: {
      attachments: [`/demo/${data.code}/ban-an.pdf`, `/demo/${data.code}/muc-luc.pdf`],
  ...
  const fileSeeds = [
    {
      ...
      details: { court: 'TAND tỉnh Sông Bé', summary: 'Hồ sơ trước khi tách tỉnh.' },
    },
    {
      code: 'DS-2021-014',
      ...
      details: { value: 450000000, summary: 'Tranh chấp nghĩa vụ thanh toán khoản vay.' },
    },
    {
      code: 'DS-2021-088',
      ...
      isLocked: true,
      boxId: boxes['BOX-002'].id,
      details: { landLot: 'Thửa 112, phường Phú Lợi', summary: 'Hồ sơ có giá trị lưu trữ lâu dài.' },
    },
    {
      code: 'HNGD-2022-031',
      ...
      details: { children: 1, summary: 'Giải quyết ly hôn và quyền trực tiếp nuôi con.' },
    },
    {
      code: 'KDTM-2023-009',
      ...
      details: { contractNo: 'HDMB-45/2022', summary: 'Tranh chấp giao hàng và thanh toán.' },
    },
    {
      code: 'HC-2024-006',
      ...
      details: { decisionNo: '45/QD-XPHC', summary: 'Hồ sơ đang được mượn quá hạn.' },
    },
    {
      code: 'HS-2025-021',
    ...
    },
    {
      ...
      details: { summary: 'Hồ sơ dùng để test tìm kiếm theo loại án lao động.' },
    },
  ]
...
async function seedBorrowSlips(
  ...
  const slips = [
    ...
    },
    {
      ...
      status: 'RETURNED',
      lenderId: users.coordinator.id,
      items: [{ fileCode: 'KDTM-2023-009', status: 'RETURNED', returnedDate: daysFromNow(-1) }],
    },
  ]
  ...
  for (const slip of slips) {
    ...
    await prisma.borrowSlipEvent.deleteMany({ where: { borrowSlipId: existing.id } })
    await prisma.borrowItem.deleteMany({ where: { borrowSlipId: existing.id } })
  ...
  await prisma.auditLog.createMany({
    data: [
      ...
      },
      {
        ...
        target: 'SeedDemo',
        detail: { note: 'Dữ liệu mẫu phục vụ test phân quyền, hồ sơ, mượn trả, audit.' },
        ipAddress: '127.0.0.1',
